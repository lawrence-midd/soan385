...
We'll use the `week9.csv` file again today. Load it as a data frame called `week9` and load the usual packages:
...
We have some cells with low expected frequencies here, so we need to use a different test. If the expected frequency in any cell is less than 5, use Fisher's Exact Test rather than the chi-squared test. Fisher's Exact Test uses the probability foundations we have seen previously to create 2000 samples of the variables we are testing and uses the principles of sampling distributions to estimate the p-value:

```{r}
fisher.test(immigrant$class, immigrant$marital, simulate.p.value = TRUE) 
```
...
What about `class` and `courts`? (Assume for now that class can be ordered...)
